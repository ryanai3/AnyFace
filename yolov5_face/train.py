      ...
      if rank in [-1, 0]:
        ...
        if plots and ni < 3:
          ...
          wandb.log(
            {
              "Mosaics": [
                wandb.Image(str(x), caption=x.name) for x in save_dir.glob("train*.jpg")
              ]
            }
          )
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    "--weights", type=str, default="weights/yolov5s.pt", help="initial weights path"
  )
  parser.add_argument(
  ...
  parser.add_argument(
    "--multi-scale", action="store_true", default=False, help="vary img-size +/- 50%%"
  )
  parser.add_argument(
