  ...
  def forward(self, x):
    ...
    for i in range(self.nl):
      ...
      if not self.training:  # inference
        ...
        y[..., 5:7] = (
          y[..., 5:7] * 2.0 - 0.5 + self.grid[i].to(x[i].device)
        ) * self.stride[
          i
        ]  # xy
        y[..., 7:9] = (y[..., 7:9] * 2) ** 2 * self.anchor_grid[i]  # wh
        #        y[..., 7:9] = x[i][..., 7:9] ** 2
        #        y[..., 7:9] = (x[i][..., 7:9] * y[..., 2:4])
...
class Model(nn.Module):
  ...
  def forward(self, x, augment=False, profile=False, aug_setting=1, feature_vis=False):
    if augment:
      ...
      if aug_setting == 1:
        ...
        s = [1, 0.83, 0.67, 1, 0.83, 0.67]  # scales
        f = [None, None, None, 3, 3, 3]  # flips (2-ud, 3-lr)
      y = []  # outputs
      for si, fi in zip(s, f):
        xi = scale_img(x.flip(fi) if fi else x, si, same_shape=True)
        yi = self.forward_once(xi, feature_vis=feature_vis)[0]  # forward
      ...
      return self.forward_once(
        x, profile, feature_vis=feature_vis
