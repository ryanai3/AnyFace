...
if __name__ == "__main__":
  ...
  parser.add_argument(
    "--simplify", action="store_true", default=False, help="simplify onnx"
  )
  parser.add_argument(
    "--dynamic",
    action="store_true",
    default=False,
    help="enable dynamic axis in onnx model",
  )
  parser.add_argument(
    "--onnx2pb", action="store_true", default=False, help="export onnx to pb"
  )
  parser.add_argument(
  ...
  # =======================TensorRT=================================
  parser.add_argument(
    "--onnx2trt", action="store_true", default=False, help="export onnx to tensorrt"
  )
  parser.add_argument(
    "--fp16_trt", action="store_true", default=False, help="fp16 infer"
  )
  # ================================================================
  ...
  if opt.onnx2trt:
    ...
    ONNX_to_TRT(
      onnx_model_path=f,
      trt_engine_path=f.replace(".onnx", ".trt"),
      fp16_mode=opt.fp16_trt,
    )

  # PB export
  if opt.onnx2pb:
    print(
      "download the newest onnx_tf by https://github.com/onnx/onnx-tensorflow/tree/master/onnx_tf"
    )
