...
def detect(model, img0):
  ...
  if pred is not None:
    ...
    pred[:, 5:15] = scale_coords_landmarks(
      img.shape[2:], pred[:, 5:15], img0.shape
    ).round()
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    ...
    nargs="+",
    type=str,
    default="runs/train/exp5/weights/last.pt",
  ...
  parser.add_argument(
    "--dataset_folder",
    default="../WiderFace/val/images/",
    type=str,
    help="dataset path",
  )
  parser.add_argument(
    "--folder_pict",
    default="/yolov5-face/data/widerface/val/wider_val.txt",
    type=str,
    help="folder_pict",
  )
  ...
  with torch.no_grad():
    ...
    for image_path in tqdm(paths.list_files(testset_folder, validExts="jpg")):
      ...
      with open(save_name, "w") as fd:
        ...
        for box in boxes:
          fd.write(
            "%d %d %d %d %.03f"
            % (box[0], box[1], box[2], box[3], box[4] if box[4] <= 1 else 1)
            + "\n"
          )
    print("done.")
